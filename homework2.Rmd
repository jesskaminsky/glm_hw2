...
**Plots of the Linear Model Prediciting Buchanan Votes from all other variables in dataset**
```{r}
par(mfrow = c(2,2))
...
**Plots of the Linear Model Prediciting Log of Buchanan Votes from Log of all other variables in dataset**
```{r}
par(mfrow = c(2,2))
plot(full_model)
```
...
To assess leverage in our model, we will look at the diagonal elements of the hat matrix. As a general rule, large hat values are those greater than (2*number of predictors) / sample size. In our model, 2p/n = .529. None of the hat values are larger than this. Also, when looking at the boxplot of hat values, we can see that none of the values are extremely different from the others. There is not evidence of leverage in this model.
